// Icons: renderização após carregamento
...
// Helpers UI
function setStatus(s) { els.statusChip.textContent = s; }
...
function loadFramework(name) {
  ...
  // reatribui títulos/hints/colunas preservando ordem
  schema.forEach((s, i) => {
    state.blocks[i].title = s.title;
    state.blocks[i].hint = s.hint || "";
    state.blocks[i].span = (s.span || 6);
  });
  renderBlocks();
  setStatus('editando');
