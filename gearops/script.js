...
function showMainApp() {
    ...
    showModule('dashboard', null); // Pass null as event since there's no click event
    initializeCharts();
}
...
function showModule(moduleName, event) {
    ...
    // Update sidebar active state only if there's an event (click)
    if (event) {
        ...
        // If no event, find the sidebar item for the current module and activate it
        document.querySelectorAll('.sidebar-item').forEach(item => {
            item.classList.remove('bg-blue-50', 'dark:bg-blue-900/20', 'text-blue-600', 'dark:text-blue-400');
            item.classList.add('text-gray-700', 'dark:text-gray-300');
            
            // Check if this item corresponds to the current module
            const itemText = item.querySelector('span').textContent.toLowerCase();
            const moduleMap = {
                'dashboard': 'dashboard',
                'projetos dealegear': 'projects',
                'financeiro': 'finance',
                'marketing': 'marketing',
                'societário': 'partners',
                'calendário': 'calendar',
            ...
            if (moduleMap[itemText] === moduleName) {
